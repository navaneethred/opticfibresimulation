...
def simulate_output_current(fiber_length, bend_radius, ambient_temp,
...
def run_length_simulation():
    ...
    ax_length.clear()
    ax_length.plot(fiber_lengths, output_currents, label="Output Current (µA)")
    ax_length.set_xlabel("Fiber Length (km)")
    ax_length.set_ylabel("Output Current (µA)")
    ax_length.set_title(f"Output Current vs Fiber Length ({fiber_type})")
    ax_length.grid(True)
    ax_length.legend()
    canvas_length.draw()
...
def run_bending_simulation():
    ...
    total_losses = []
    for R in bend_radii:
        I_out, tot_loss = simulate_output_current(fixed_length, R, ambient_temp,
                                                  att_coeff, base_bend_loss, ideal_bend_rad)
        output_currents.append(I_out)
        total_losses.append(tot_loss)
    ...
    ax_bending.clear()
    ax_bending.plot(bend_radii, output_currents, color="green", label="Output Current (µA)")
    ax_bending.set_xlabel("Bending Radius (cm)")
    ax_bending.set_ylabel("Output Current (µA)")
    ax_bending.set_title(f"Output Current vs Bending Radius (Fixed Length = {fixed_length} km, {fiber_type})")
...
def run_turns_simulation():
    ...
    for n in n_turns_array:
        I_out, tot_loss = simulate_output_current(fixed_length, bend_radius, ambient_temp,
    ...
    ax_turns.clear()
    ax_turns.plot(n_turns_array, output_currents, color="red", label="Output Current (µA)")
    ax_turns.set_xlabel("Number of Turns")
    ax_turns.set_ylabel("Output Current (µA)")
    ax_turns.set_title(f"Output Current vs Number of Turns ({fiber_type})")
    ax_turns.grid(True)
    ax_turns.legend()
    canvas_turns.draw()
...
def save_entire_graph():
    ...
    if file_path:
        # Save the entire Notebook's figures in one image (here, we save the three subplots vertically)
        fig_entire, (ax_entire1, ax_entire2, ax_entire3) = plt.subplots(3, 1, figsize=(6, 12))
        # Length simulation
        if length_sim_data is not None:
            fiber_lengths, out_curr, _ = length_sim_data
            ax_entire1.plot(fiber_lengths, out_curr, label="Output Current (µA)")
            ax_entire1.set_xlabel("Fiber Length (km)")
            ax_entire1.set_ylabel("Output Current (µA)")
            ax_entire1.set_title(f"Output Current vs Fiber Length ({fiber_type_var.get()})")
            ax_entire1.grid(True)
            ax_entire1.legend()
        # Bending simulation
        if bending_sim_data is not None:
            bend_radii, out_curr, _ = bending_sim_data
            ax_entire2.plot(bend_radii, out_curr, color="green", label="Output Current (µA)")
            ax_entire2.set_xlabel("Bending Radius (cm)")
            ax_entire2.set_ylabel("Output Current (µA)")
            ax_entire2.set_title(f"Output Current vs Bending Radius ({fiber_type_var.get()})")
            ax_entire2.grid(True)
            ax_entire2.legend()
        # Turns simulation
        if turns_sim_data is not None:
            n_turns_array, out_curr, _ = turns_sim_data
            ax_entire3.plot(n_turns_array, out_curr, color="red", label="Output Current (µA)")
            ax_entire3.set_xlabel("Number of Turns")
            ax_entire3.set_ylabel("Output Current (µA)")
            ax_entire3.set_title(f"Output Current vs Number of Turns ({fiber_type_var.get()})")
...
# Ambient temperature input (common)
...
# Make the grid in root responsive
for i in range(2):
